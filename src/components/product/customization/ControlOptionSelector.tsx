                ...
                {options.map((option) => (
                    ...
                    >
                        <button
                            ...
                            className={`relative flex items-center gap-3 h-full w-full border rounded-[12px] p-3 transition-all hover:border-[#b8c7df] text-left md:flex-col md:items-stretch md:text-center ${selectedOption === option.id
                                    ? 'border-[#335c99] bg-[#eef2f8]'
                                    : 'border-[#cbd6e6] bg-white'
                                }`}
                        >
                            {option.image && (
                                <div className="relative h-16 w-16 shrink-0 md:h-[70px] md:w-full md:mb-2 bg-[#e7eef8] rounded overflow-hidden flex items-center justify-center">
                                    <Image
                                        src={option.image}
                                        alt={option.name}
                                        fill
