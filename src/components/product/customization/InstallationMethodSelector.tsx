                ...
                {options.map((option) => (
                    ...
                    >
                        <button
                            ...
                            className={`relative flex items-center gap-3 h-full w-full border rounded-[12px] p-3 md:p-4 transition-all hover:border-[#b8c7df] text-left md:flex-col md:items-stretch ${selectedMethod === option.id
                                    ? 'border-[#335c99] bg-[#eef2f8]'
                                    : 'border-[#d9dfeb] bg-white'
                                }`}
                        >
                            {option.image && (
                                <div className="relative h-16 w-16 shrink-0 md:h-[120px] md:w-full md:mb-3 bg-[#eef2f8] rounded overflow-hidden flex items-center justify-center">
                                    <Image
                                        src={option.image}
                                        alt={option.name}
                                        fill
